<h1 align="center">👋 Hey, I'm Hicham</h1>

<p align="center">
  <strong>Full Stack Developer</strong> • Engineer • Curious builder<br/>
  Turning ideas into clean & useful web products
</p>

<hr/>

<h1>👀 About me</h1>

<p>
  I'm a <strong>full stack developer</strong> and software engineer, graduated from <strong>Polytech Tours</strong> 
  and holding a <strong>Master’s degree in Computer Science</strong> from the University of 
  <strong>Sherbrooke (Canada)</strong>.
</p>

<p>
  I enjoy working on <strong>end-to-end web projects</strong>: from designing APIs and data models
  to building user-facing features and deploying them in production.
</p>

<p>
  I'm especially interested in <strong>clean code</strong>, <strong>scalable systems</strong> 
  and <strong>products that actually suit to clients</strong>.
</p>

<hr/>

<h1>🚀 What I like building</h1>

<ul>
  <li>🌐 Web applications (front + back)</li>
  <li>⚙️ APIs & backend services</li>
  <li>🔐 Auth systems & permissions</li>
  <li>⚡ Real-time features (WebSockets)</li>
  <li>☁️ CI/CD & cloud deployments</li>
</ul>

<hr/>

<h1>🚧 Projects</h1>

<p>
  A selection of projects I've worked on — side projects, experiments and real-world applications.
</p>

<br/>

<!-- PROJECT 1 -->
<div>
  <h2><a href="https://app.floworkerz.xyz/">💻 Floworkerz (2025)</a></h2>
  
  
https://github.com/user-attachments/assets/fe43428f-49f0-46a0-9b0b-0bcd8cefc101


  <p>
    Floworkerz is a collaborative productivity platform designed to help people stay focused while working or studying together. The app is built around real-time focus sessions where users can join shared work rooms, follow a timer, interact via live chat, and stay motivated through a collective effect.
  </p>

  <p>
    <strong>Tech:</strong> React • Node.js • PostgreSQL • AWS • Websockets • TailwindCSS
  </p>
</div>

<br/><br/>

<!-- PROJECT 2 -->
<div>
  <h2>🥖 Cooking Overtime ! (13th edition of the Ubisoft Game Lab Competition)</h2>

  <p align="center">
    <img width="1920" height="1080" alt="ImagePresentation" src="https://github.com/user-attachments/assets/821104dc-049f-42f4-aa13-f8de0c4cac43" />
  </p>

  <p>
    I had the opportunity to participate in the 13th edition of the Ubisoft Game Lab Competition as a member of the University of Sherbrooke team. Our team of eight was tasked with creating an arcade-style game under specific constraints. We developed an original roguelike concept centered around cooking, set in a playful universe where food both attacks the player and enhances their stats.

My main responsibilities included creating visual assets and contributing to level design. I also took part in brainstorming and refining ideas related to theme integration, gameplay mechanics, and procedural generation.
  </p>

  <p align="center">
    <a href="https://www.youtube.com/watch?v=jgIqBRt-SDI">Here's a Gameplay video</a>
  </p>
</div>

<br/><br/>

<!-- PROJECT 3 -->
<div>
  <h2>📱 PokemonGo-like (2022)</h2>

  <p align="center">
    <img width="20%" height="20%" alt="POLYPOKE_logo" src="https://github.com/user-attachments/assets/e17d36ac-8329-4bc7-8d59-16924f36dd28" />
  </p>

  <p>
    The aim of the application is to reproduce to some extent the PokémonGO application. The aim is to put into practice our knowledge of setting up a NodeJS server and developing Android applications. This project is part of my Internet and Mobility course. This project is constituted of tree parts: the server, the android application and a web site.
  </p>

  <p align="center">
    <img width="768" height="758" alt="image2" src="https://github.com/user-attachments/assets/50bbd702-9700-4345-9f3b-7745229e11b1" />
  </p>

  <p align="center">
    <strong>Some screenshot of the android application.</strong>
  </p>
  <p align="center">
    <img width="645" height="304" alt="image1" src="https://github.com/user-attachments/assets/fd2fdb75-845c-49ce-bd36-94e999ef47d9" />
  </p>

</div>

<br/><br/>

<!-- PROJECT 4 -->
<div>
  <h2>💡 Image synthesis - illumination system (2023)</h2>

  <p>
    From a C++ 3d engine base, I added image rendering to achieve different illumination of the scene.
  </p>

  <h3>Ray tracer</h3>
  <p align="center">
    <img width="1299" height="645" alt="RayTracer" src="https://github.com/user-attachments/assets/05b3e1f8-dc29-424b-8fcd-d80aab79e608" />
  </p>
  
  <p align="center">
    <strong>left: 3D scene without rendering, right: image generated by ray tracing</strong>
  </p>

  <p>
    A ray tracer that generates images with shadows, reflections and refractions (Fresnel) based on custom 3d models and textures.
  </p>

  <h3>Photon mapping</h3>
  <p align="center">
    <img width="1423" height="471" alt="photonMapping" src="https://github.com/user-attachments/assets/bf7e1a94-b105-460a-9110-f7acf17ce5a4" />
  </p>
  
  <p align="center">
    <strong>left: 3D scene without rendering, middle: the scene after photon mapping, right: image generated by ray tracing (taking photons accumulation into account)</strong>
  </p>

  <p>
    A photon mapping to generate photons that simulate the behavior of light as it interacts with different materials. For example, when generating caustics.
  </p>

  <h3>Radiosity (Gauss-Seidel)</h3>
  <p align="center">
    <img width="862" height="929" alt="radiosity" src="https://github.com/user-attachments/assets/8c7d829b-a33c-4f79-9247-036929797e79" />
  </p>
  
  <p align="center">
    <strong>top: 3D scene without rendering, bottom: 3D scene after using radiosity</strong>
  </p>
  <p>
    Radiosity lighting calculation system using the Gauss-Seidel method. This simulates the physical behavior of light by calculating the quantities received and emitted on a surface.
  </p>

</div>

<hr/>

<h2>🛠️ Tech stack</h2>

<p><strong>Frontend</strong></p>
<ul>
  <li>React • Angular • TypeScript</li>
  <li>HTML • CSS</li>
</ul>

<p><strong>Backend</strong></p>
<ul>
  <li>Node.js</li>
  <li>Java / Spring Boot</li>
</ul>

<p><strong>Databases</strong></p>
<ul>
  <li>PostgreSQL • SQL</li>
</ul>

<p><strong>Tools & Cloud</strong></p>
<ul>
  <li>Git • GitLab CI</li>
  <li>AWS • Bash</li>
</ul>

<hr/>

<h2>📌 Right now</h2>

<ul>
  <li>🔍 Actively looking for new opportunities</li>
  <li>💼 Full Stack / Backend roles</li>
  <li>🌍 Open to France & international opportunities</li>
</ul>

<hr/>

<h2>📫 Let's connect</h2>

<ul>
  <li>📧 <a href="mailto:hicham.moustaqim@gmail.com">hicham.moustaqim@gmail.com</a></li>
  <li>💼 <a href="https://www.linkedin.com/in/hicham-moustaqim-898560220" target="_blank">LinkedIn</a></li>
</ul>

<hr/>

<p align="center">
  ⭐ Feel free to explore my repositories — and reach out if you'd like to collaborate!
</p>
